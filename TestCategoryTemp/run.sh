...
RUNNING=$(docker inspect --format="{{ .State.Running }}" TestCategoryTemp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    echo "Stopping... with title TestCategoryTemp"
    docker rm -f TestCategoryTemp
fi
