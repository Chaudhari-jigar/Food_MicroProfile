...
RUNNING=$(docker inspect --format="{{ .State.Running }}" SampleProfileApp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    echo "Stopping... with title SampleProfileApp"
    docker rm -f SampleProfileApp
fi
