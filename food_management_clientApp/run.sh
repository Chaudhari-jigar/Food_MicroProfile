...
RUNNING=$(docker inspect --format="{{ .State.Running }}" food_management_clientApp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    docker run \
     -d \
     --rm \
     --name food_management_clientApp \
     -p 8080:8080 \
     -p 8181:8181 \
     -p 4848:4848 \
    ...
    echo "Stopping... with title food_management_clientApp"
    docker rm -f food_management_clientApp
fi
