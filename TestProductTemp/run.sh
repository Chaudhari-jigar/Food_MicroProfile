...
RUNNING=$(docker inspect --format="{{ .State.Running }}" TestProductTemp 2> /dev/null)
if [ $? -eq 1 ] || [ "$RUNNING" == "false" ]; then
    ...
    echo "Stopping... with title TestProductTemp"
    docker rm -f TestProductTemp
fi
